...
def build_stem(feature_dim, module_dim, num_layers=2, with_batchnorm=True):
  ...
  for i in range(num_layers):
    ...
    if with_batchnorm:
      layers.append(tf.keras.layers.batch_normalization(module_dim))
...
def build_classifier(module_C, module_H, module_W, num_answers,
  ...
  if proj_dim is not None and proj_dim > 0:
    ...
    if with_batchnorm:
      layers.append(tf.keras.layers.batch_normalization(proj_dim))
    layers.append(tf.keras.layers.ReLU())
  ...
  for next_dim in fc_dims:
    layers.append(tf.keras.layers.Linear(prev_dim, next_dim))
    if with_batchnorm:
      layers.append(tf.keras.layers.batch_normalization(next_dim))
    layers.append(tf.keras.layers.ReLU())
    if dropout > 0:
...
class ModuleNet():
  ...
  def forward(self, x, program):
    ...
    elif type(program) is tf.Variable and program.dim() == 2:
